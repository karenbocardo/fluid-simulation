...
how to run: $ python fluid.py src.json
"""
import numpy as np
import math
import sys
...
def main():    
    ...
    def update_im(i):
        ...
        for obj in objs:
            ...
            h, w = obj['size']['height'], obj['size']['width']
            density = obj['density']

            inst.density[y:y+h, x:x+w] = density
        ...
        for source in sources:
            ...
            vel_x, vel_y = source['velo']['x'], source['velo']['y']
            behvr = source['velo']['behaviour']

            # We add new density creators in here
            # inst.density[14:17, 14:17] += 100  
            inst.density[y:y+size, x:x+size] += density  # add density into a 3*3 square
            ...
            # behaviours
            if behvr == 1:
                inst.velo[y+h, x+h] = [np.cos(f  * i), np.sin(f  * i)]
            elif behvr == 2:
                inst.velo[y+h, x+h] = [-vel_y, vel_x * np.sin(0.5 * i)]
            else: 
                inst.velo[y+h, x+h] = [vel_y, vel_x]
            #
    ...
    # plot vector field
    q = plt.quiver(inst.velo[:, :, 1], inst.velo[:, :, 0], scale=100, angles='xy')
    anim = animation.FuncAnimation(fig, update_im, interval=0)
    anim.save(f"{vid_name}.mp4", fps=30, extra_args=['-vcodec', 'libx264'])
    #plt.show()
